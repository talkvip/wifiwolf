	...
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/user/save" method="post" class="form-horizontal">
		...
		<div class="form-group">
			...
			<shiro:hasRole name="admin">
				<form:input path="username" class="form-control" htmlEscape="false" maxlength="50" />
			</shiro:hasRole>
			<shiro:lacksRole name="admin">
				<form:input path="username"  class="form-control" htmlEscape="false" maxlength="50" disabled="true" />
			</shiro:lacksRole>
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<div class="col-xs-6 col-sm-6">
				<form:password path="plainPassword" class="form-control" htmlEscape="false" maxlength="50" />
				<c:if test="${not empty user.id}"><span class="help-inline">若不修改密码，请留空。</span></c:if>
			</div>
		</div>
		<div class="form-group">
			<label class="col-xs-2 col-sm-2 control-label" for="confirmNewPassword">确认密码:</label>
			<div class="col-xs-6 col-sm-6">
				<input id="confirmNewPassword" class="form-control" name="confirmNewPassword" type="password" value="" maxlength="50" minlength="3" equalTo="#plainPassword"/>
			</div>
		</div>
		<div class="form-group">
			<label class="col-xs-2 col-sm-2 control-label" for="phone">电话号码:</label>
			<div class="col-xs-6 col-sm-6">
				<form:input path="phone" class="form-control" htmlEscape="false" maxlength="16"/>
			</div>
		</div>
		<div class="form-group">
			<label class="col-xs-2 col-sm-2 control-label" for="email">邮箱:</label>
			<div class="col-xs-6 col-sm-6">
				<form:input path="email" class="form-control" htmlEscape="false" maxlength="100"/>
			</div>
		</div>
		<div class="form-group">
